...
export class AutoFetchService {
  ...
  /**
   * Start auto-fetch service with configured interval
   */
  start(): void {
  ...
  updateSettings(settings: Partial<AutoFetchSettings>): void {
    console.log('Updating auto-fetch settings:', settings)

    // Update store
    Object.entries(settings).forEach(([key, value]) => {
      this.setSetting(key as keyof AutoFetchSettings, value as any)
    })

    // Restart if currently running and interval changed
    if (this.timerId && settings.intervalMinutes) {
      console.log('Interval changed, restarting auto-fetch')
      this.stop()
      this.start()
    }
  }
  ...
  /**
   * Manually trigger fetch outside of schedule
   */
  ...
  getStatus(): AutoFetchStatus {
    const nextFetchIn = this.calculateNextFetchIn()

    return {
      enabled: this.getSetting('enabled'),
      intervalMinutes: this.getSetting('intervalMinutes'),
      lastFetchTimestamp: this.getSetting('lastFetchTimestamp'),
  ...
  private calculateNextFetchIn(): number | null {
    if (!this.timerId || !this.lastFetchTime) {
      return null
    }

    const intervalMs = this.getSetting('intervalMinutes') * 60 * 1000
  ...
  private async fetchEmails(): Promise<FetchResult> {
    ...
    try {
      // Trigger fetch via renderer process
  ...
  /**
   * Show desktop notification for new emails
   */
  private showNotification(result: FetchResult): void {
    if (!this.getSetting('notificationsEnabled')) {
      return
    }

    const { newEmailCount, stats } = result
    const autoLabeledText =
      stats.autoLabeled > 0 ? ` (${stats.autoLabeled} auto-labeled)` : ''

    new Notification({
      title: 'Gmail Labeler',
      body: `${newEmailCount} new email${newEmailCount !== 1 ? 's' : ''}${autoLabeledText}`,
      silent: false,
    }).show()

    console.log('Notification shown for new emails')
  }
  ...
  private notifyRenderer(event: string, data?: any): void {
    this.mainWindow.webContents.send('auto-fetch:status-changed', {
      event,
      data,
